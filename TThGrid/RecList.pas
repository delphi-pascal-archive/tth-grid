{ RecList : auteur ThWilliam
     TCustomRecList, TRecordsList : 19/04/2008
     modifications 20/04/08 : correction DoFindKey, Add
     modification 29/04/08 : ajout de TRecordsKist.EditField}
...
type
  ...
  private
    ...
    FArraySortValues: array of string;
    ...
    function SetASortValues(const A: array of string): boolean;
  protected
  ...
  public
    ...
    function Insert(const A: array of string; ARecord: integer): integer;
    function Edit(const A: array of string; ARecord: integer): integer;
    function EditField(AField: integer; ARecord: integer; AValue: string): integer;
    procedure Delete(ARecord: Integer);
...
var
  ARecord: integer;
begin
  if IsSorted and SetASortValues(A) then
  begin
     DoFindKey(FArraySortValues, ARecord, FSortOptions);
...
begin
  ...
  SetLength(FArraySortValues, Length(FArraySortKey));
end;

function TCustomRecList.SetASortValues(const A: array of string): boolean;
{fonction utilisée en interne par Add sur liste triée
 A = tous les champs d'un enregistrement
 Reprise à partir de A des champs triés (stockage dans FArraySortValues)}
var
  I: integer;
begin
  try
    for I:= 0 to Length(FArraySortKey) - 1 do
      FArraySortValues[I]:= A[FArraySortKey[I]];
    Result:= true;
  except
    Result:= false;
  end;
...
begin
  ...
  begin
     ...
     for N:= 0 to CF - 1 do
     begin
        C:= InternalCompareValues(A[N], GetFieldValue(FArraySortKey[N], I),
...
function TRecordsList.EditField(AField: integer; ARecord: integer; AValue: string): integer;
{EditField permet une modification d'un champ plus facilement que Edit.
 AField peut être plus grand que l'indice du dernier champ actuel dans l'enregistrement.
 Dans ce cas, il y a création des champs manquants initialisés à ''}
var
  A: array of string;
  N, I: integer;
begin
  N:= FieldsCount[ARecord];
  if AField >= N then SetLength(A, AField + 1)
    else SetLength(A, N);
  try
     for I:= 0 to N - 1 do
        A[I]:= GetFieldValue(I, ARecord);
     for I:= N to Length(A) - 1 do A[I]:= '';
     A[AField]:= AValue;
     Result:= inherited Edit(A, ARecord);
  finally
     SetLength(A, 0);
